...
class Hom_budget_hom extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Hom_budget_hom');
        $this->controller_url='hom_budget_hom';
    ...
    private function system_get_edit_items()
    {
        ...
        $this->db->from($this->config->item('table_bms_di_forward').' area_forward');
        $this->db->select('area_forward.status_forward_budget,area_forward.division_id');
        $this->db->where('area_forward.year_id',$year_id);
        $this->db->where_in('area_forward.division_id',$area_ids);
        ...
        foreach($results as $result)
        {
            $area_budget[$result['variety_id']][$result['division_id']][$result['year_index']]=$result['quantity_budget'];
        }
    ...
    private function system_forward()
    {
        ...
        $crop_type_id=$this->input->post('crop_type_id');
        $forwarded=false;
        $result=Query_helper::get_info($this->config->item('table_bms_hom_forward'),array('id','status_forward_budget'),array('year_id ='.$year_id,'crop_type_id ='.$crop_type_id),1);
        if($result && $result['status_forward_budget']==$this->config->item('system_status_yes'))
        {
            $forwarded=true;
        }
