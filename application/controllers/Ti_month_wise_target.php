...
class Ti_month_wise_target extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Ti_month_wise_target');
        $this->controller_url='ti_month_wise_target';
    ...
    private function system_search()
    {
        ...
        {
            $data['title']="TI Month Wise Target Search";
    ...
    private function system_edit($id)
    {
        ...
        //target status
        $this->db->from($this->config->item('table_bms_ti_monthwise_target').' mt');
        $this->db->select('mt.*');
        $this->db->select('forward.status_forward_month_wise_target,forward.date_forward_month_wise_target,forward.user_forward_month_wise_target');
        ...
        $data['month_wise_target_info']['status_target']='Not Done';
        $data['month_wise_target_info']['date_target']='N/A';
        $data['month_wise_target_info']['user_target']='N/A';
        $data['month_wise_target_info']['date_forward']='N/A';
        $data['month_wise_target_info']['user_forward']='N/A';
        if($result)
        {
            ...
            if($result['user_forward_month_wise_target']>0)
            {
                $user_ids[$result['user_forward_month_wise_target']]=$result['user_forward_month_wise_target'];
            }
            ...
            foreach($users as $u)
            {
                ...
                {
                    $data['month_wise_target_info']['user_target']=$u['name'];
                }
                if($u['user_id']==$result['user_forward_month_wise_target'])
                {
                    $data['month_wise_target_info']['user_forward']=$u['name'];
                }
            }


            if($result['status_forward_month_wise_target'] && $result['status_forward_month_wise_target']==$this->config->item('system_status_yes'))
            {
                $data['month_wise_target_info']['status_target']='Forwarded';
                $data['month_wise_target_info']['date_forward']=System_helper::display_date_time($result['date_forward_month_wise_target']);
                $data['month_wise_target_info']['date_target']=System_helper::display_date_time($result['date_targeted']);
            }
            elseif($result['revision']!=0)
            {
                $data['month_wise_target_info']['status_target']='Not Forwarded';
                $data['month_wise_target_info']['date_target']=System_helper::display_date_time($result['date_targeted']);
            }
        }
        $data['title']='TI Month Wise Target';
        ...
        for($i=0;$i<12;$i++)
        {
            ...
            $data['months'][$current_month]=$this->lang->line('LABEL_MONTH_'.$current_month);
        }
    ...
    private function system_get_edit_items()
    {
        ...
        //getting TI previous years data
        ...
        $this->db->join($this->config->item('table_login_setup_classification_varieties').' v','v.id = mt.variety_id','INNER');
        $this->db->where('v.crop_type_id',$crop_type_id);
        $this->db->limit($this->config->item('num_year_previous_sell'),0);
        $results=$this->db->get()->result_array();
        foreach($results as $result)
        {
        ...
        //getting zi assign target forward to ti data
        ...
        //getting ti monthwise target forward data
        $this->db->select('status_forward_month_wise_target');
        ...
        if($result && $result['status_forward_month_wise_target']==$this->config->item('system_status_yes'))
        {
        ...
        foreach($results as $result)
        {
            ...
            for($month=1;$month<=12;$month++)
            {
                foreach($years_previous as $year)
                {
                    ...
                    if(isset($items_previous_years[$result['id']][$year['value']]))
                    {
                        if($zi_forward_status_assign_target_ti)
                        {
                            $item['year'.$year['value'].'_month'.$month.'_quantity_target']=$items_previous_years[$result['id']][$year['value']]['target_quantity_'.$month];
                        }
                        else
                        {
                            $item['year'.$year['value'].'_month'.$month.'_quantity_target']='N/F';
                        }
                    }
                    else
                    {
                        $item['year'.$year['value'].'_month'.$month.'_quantity_target']='N/A';
                    }
                }
                if(isset($items_current_ti[$result['id']]))
                {
                    $item['month'.$month.'_quantity_target']=$items_current_ti[$result['id']]['target_quantity_'.$month];
                }
                else
                {
    ...
    private function system_save()
    {
        if((isset($this->permissions['action1']) && ($this->permissions['action1']==1))||(isset($this->permissions['action2']) && ($this->permissions['action2']==1))||(isset($this->permissions['action3']) && ($this->permissions['action3']==1)))
        {
            ...
            $result=Query_helper::get_info($this->config->item('table_bms_ti_forward'),array('status_forward_month_wise_target'),array('year_id ='.$year_id,'crop_type_id ='.$crop_type_id,'territory_id ='.$territory_id),1);
            //print_r($result);exit;
            if($result && $result['status_forward_month_wise_target']==$this->config->item('system_status_yes'))
            {
                $forwarded=true;
            ...
            foreach($items as $variety_id=>$item)
            {
                if(isset($items_current[$variety_id]))
                {
                    ...
                    for($i=1;$i<=12;$i++)
                    {
                        if($items_current[$variety_id]['target_quantity_'.$i]!=$item['target_quantity_'.$i])
                        {
                            $changed=true;
                            break;
                        }
                    ...
                    if($changed)
                    {
                        ...
                        if($editable)
                        {
                            ...
                            for($i=1;$i<=12;$i++)
                            {
                                $this->db->set('target_quantity_'.$i,$item['target_quantity_'.$i]);
                            }
                            ...
                            $this->db->update($this->config->item('table_bms_ti_monthwise_target'));
                        }
                    }
                }
                else
                {
                    if((isset($this->permissions['action3']) && ($this->permissions['action3']==1))||(!$forwarded))
                    {
                        ...
                        Query_helper::add($this->config->item('table_bms_ti_monthwise_target'),$item,false);
                    }
                }
            }
    ...
    private function system_forward()
    {
        ...
        $territory_id=$this->input->post('territory_id');
        $forwarded=false;
        $result=Query_helper::get_info($this->config->item('table_bms_ti_forward'),array('id','status_forward_month_wise_target'),array('territory_id ='.$territory_id,'year_id ='.$year_id,'crop_type_id ='.$crop_type_id),1);
        if($result && $result['status_forward_month_wise_target']==$this->config->item('system_status_yes'))
        {
            $forwarded=true;
        }
        if($forwarded)
        {
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>'');
            $ajax['system_message']="This Month Wise Target already Forwarded";
            $this->json_return($ajax);
        }
        else if((isset($this->permissions['action1']) && ($this->permissions['action1']==1))||(isset($this->permissions['action2']) && ($this->permissions['action2']==1))||(isset($this->permissions['action3']) && ($this->permissions['action3']==1)))
        {
            ...
            {
                $data=array();
                $data['status_forward_month_wise_target']=$this->config->item('system_status_yes');
                $data['date_forward_month_wise_target'] = $time;
                $data['user_forward_month_wise_target'] = $user->user_id;
