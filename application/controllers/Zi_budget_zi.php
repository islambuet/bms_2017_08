...
class Zi_budget_zi extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Zi_budget_zi');
        $this->controller_url='zi_budget_zi';
    ...
    private function system_edit($id)
    {
        ...
        $this->db->where('bud.zone_id',$reports['zone_id']);
        ...
        $this->db->join($this->config->item('table_bms_zi_forward').' forward','forward.year_id = '.$reports['year_id'].' and forward.zone_id = '.$reports['zone_id'].' and forward.crop_type_id = '.$reports['crop_type_id'],'LEFT');
    ...
    private function system_get_edit_items()
    {
        ...
        //get territory ids
        $results=Query_helper::get_info($this->config->item('table_login_setup_location_territories'),array('id value','name text'),array('zone_id ='.$zone_id));
        $areas=array();
        $area_ids=array('0');
        foreach($results as $result)
        {
        ...
        $this->db->from($this->config->item('table_bms_ti_forward').' area_forward');
        $this->db->select('area_forward.status_forward_budget,area_forward.territory_id');
        $this->db->where('area_forward.year_id',$year_id);
        $this->db->where_in('area_forward.territory_id',$area_ids);
        ...
        foreach($results as $result)
        {
            $area_budget[$result['variety_id']][$result['territory_id']][$result['year_index']]=$result['quantity_budget'];
        }
        ...
        $forwarded=false;
        $result=Query_helper::get_info($this->config->item('table_bms_zi_forward'),array('status_forward_budget'),array('year_id ='.$year_id,'zone_id ='.$zone_id,'crop_type_id ='.$crop_type_id),1);
