...
class Ti_budget_ti extends Root_Controller
{
    private  $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Ti_budget_ti');
        $this->controller_url='ti_budget_ti';
    ...
    private function system_edit($id)
    {
        ...
        $this->db->select('out_info.name area_name');
        ...
        $this->db->where('bud.territory_id',$reports['territory_id']);
        ...
        $this->db->join($this->config->item('table_bms_ti_forward').' forward','forward.year_id = '.$reports['year_id'].' and forward.territory_id = '.$reports['territory_id'].' and forward.crop_type_id = '.$reports['crop_type_id'],'LEFT');
    ...
    private function system_get_edit_items()
    {
        ...
        $this->db->where_in('outlet_id',$area_ids);
        $this->db->join($this->config->item('table_login_setup_classification_varieties').' v','v.id = bud.variety_id','INNER');
        ...
        foreach($results as $result)
        {
            $area_budget[$result['variety_id']][$result['outlet_id']][$result['year_index']]=$result['quantity_budget'];
        }
        ...
        $forwarded=false;
        $result=Query_helper::get_info($this->config->item('table_bms_ti_forward'),array('status_forward_budget'),array('year_id ='.$year_id,'territory_id ='.$territory_id,'crop_type_id ='.$crop_type_id),1);
    ...
    private function system_forward()
    {
        ...
        $crop_type_id=$this->input->post('crop_type_id');
        $forwarded=false;
        $result=Query_helper::get_info($this->config->item('table_bms_ti_forward'),array('id','status_forward_budget'),array('year_id ='.$year_id,'territory_id ='.$territory_id,'crop_type_id ='.$crop_type_id),1);
        if($result && $result['status_forward_budget']==$this->config->item('system_status_yes'))
        {
            $forwarded=true;
        }
